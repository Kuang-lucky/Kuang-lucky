...
//用户名和密码都输入后，连接数据库
		...
		//构造SQL查询语句
		$sql="select * from php_admin where adminid=$adminid";
		//执行sql语句
		$result=$conn->query($sql);	
		while($row = mysqli_fetch_array($result))
		{	
			$adminpsw=$row['adminpsw'];
    ...
    <div class="container">
        <div class="row row-centered">
            <div class="col-xs-6 col-md-4 col-center-block">
                ...
                <form method="post" action="doupdate.php?adminid=<?php echo $adminid ?>">
					<!-- 输入密码 -->
                    <div class="edit input-group input-group-md ">
						<span class="input-group-addon" style="color:#28b76b ">
							密&nbsp;&nbsp;&nbsp;&nbsp;码：
						</span>
                        <input type="text" class="form-control"  name='adminpsw'  value=<?php echo $adminpsw?> />
                    </div>
					<!-- 输入用户名 -->
                    <div class="edit input-group input-group-md ">
						<span class="input-group-addon" style="color:#28b76b ">
                        ...
                        <input type="text" class="form-control" type="text" name='adminname'  value=<?php echo $adminname?> />
                    </div>
